...
# ice only
cre_interpolated_average["ice_only_net"][IWP_points > 2.3] = np.nan
cre_interpolated_average["ice_only_sw"][IWP_points > 2.3] = np.nan
cre_interpolated_average["ice_only_lw"][IWP_points > 2.3] = np.nan
...
table_cre.loc["ARTS Conn"]["SW"] = np.round(
    float((cre_interpolated_average["connected_sw"] * hist).sum().values), 1
)
table_cre.loc["ARTS Conn"]["LW"] = np.round(
    float((cre_interpolated_average["connected_lw"] * hist).sum().values), 1
)
table_cre.loc["ARTS Conn"]["Net"] = (
    table_cre.loc["ARTS Conn"]["SW"] + table_cre.loc["ARTS Conn"]["LW"]
)
table_cre.loc["ARTS No Conn"]["SW"] = np.round(
    float((cre_interpolated_average["all_sw"] * hist).sum().values), 1
)
table_cre.loc["ARTS No Conn"]["LW"] = np.round(
    float((cre_interpolated_average["all_lw"] * hist).sum().values), 1
)
