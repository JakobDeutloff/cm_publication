...
atms = cut_data(atms, atms['mask_height'])

# %% calculate cloud fractions 
iwp_bins = np.logspace(-5, 1, 50)
iwp_points = (iwp_bins[1:] + iwp_bins[:-1]) / 2
f_raw = calc_lc_fraction(atms['LWP'], connected=False)
f_raw_binned = f_raw.groupby_bins(atms['IWP'], iwp_bins).mean()
f_connected = calc_lc_fraction(atms['LWP'], connected=atms['connected']) 
f_connected_binned = f_connected.groupby_bins(atms['IWP'], iwp_bins).mean()
f_mean = f_connected_binned.where(iwp_points < 1).mean()

# %% plot lc fraction
fig, ax = plt.subplots()
ax.plot(iwp_points, f_raw_binned, label='Raw', color='purple')
ax.plot(iwp_points, f_connected_binned, color='blue', linestyle='--', label='Unconnected')
