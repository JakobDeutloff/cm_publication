...
sw_vars = xr.open_dataset(
    "/work/bm1183/m301049/icon_arts_processed/derived_quantities/sw_vars.nc"
)
aux = xr.open_dataset(
    "/work/bm1183/m301049/icon_arts_processed/fullrange_flux_mid1deg/aux.nc"
)
...
IWP_bin = [
    cre_binned.IWP_bins.isel(IWP_bins=idx_low),
    cre_binned.IWP_bins.isel(IWP_bins=idx_high),
]
...
lon_bin = [
    cre_binned.lon_bins.isel(lon_bins=idx_low),
    cre_binned.lon_bins.isel(lon_bins=idx_high),
]
...
# plot profile
fig, axes = plot_profiles_noice(
    lat=lats[0],
    lon=lons[0],
    atms=atms,
    fluxes_3d=fluxes_3d,
    fluxes_3d_noice=fluxes_3d_noice,
)
...
c = ax.scatter(
    atms["LWP"].sel(lat=slice(-30, 30)).sel(lat=slice(-30, 30)),
    albedo_lc.sel(lat=slice(-30, 30)),
    marker="o",
    s=0.2,
    c=fluxes_3d_noice.isel(pressure=-1)["allsky_sw_down"].sel(lat=slice(-30, 30)),
    cmap="viridis",
)
...
# %% check low albedo values
mask_land = aux["land sea mask"].sel(lat=slice(-30, 30)) > 0
mask_no_lc = atms["LWP"].sel(lat=slice(-30, 30)) < 1e-6

average_albedo_clear_ocean = (
    sw_vars["noice_albedo"]
    .sel(lat=slice(-30, 30))
    .where(~mask_land & mask_no_lc)
    .mean()
)
average_albedo_clear_land = (
    sw_vars["noice_albedo"].sel(lat=slice(-30, 30)).where(mask_land & mask_no_lc).mean()
)
...
# ocean values
ax.scatter(
    atms["IWP"].sel(lat=slice(-30, 30)).where(~mask_land),
    sw_vars["noice_albedo"].sel(lat=slice(-30, 30)).where(~mask_land),
    marker="o",
    s=0.4,
    color="blue",
    label="Ocean",
...
ax.axhline(
    average_albedo_clear_ocean,
    color="blue",
    label="Clearsky Average Ocean",
    linestyle="--",
)
ax.axhline(
    average_albedo_clear_land, color="k", label="Clearsky Average Land", linestyle="--"
)
ax.set_xscale("log")
...
fig.tight_layout()
fig.savefig("plots/albedo_t_vs_IWP.png", dpi=300)

# %% plot outgoing SW radiation at TOA
fig, ax = plt.subplots(
    1, 1, figsize=(8, 4), subplot_kw={"projection": ccrs.PlateCarree()}
)

pc = sw_vars["noice_albedo"].plot.pcolormesh(
    ax=ax,
...
cb = fig.colorbar(pc, extend="max", label=r"$\alpha_t$", shrink=0.8, pad=0.05)
fig.savefig("plots/albedo_t_map.png", dpi=300)
# %%
